...
def _measured_factorial(rows, model_id, *, phase, split, turns=("measured",)):
    return tuple(
        row for row in rows
...
def build_observations(rows, model_id, *, metrics=PRIMARY_METRICS, phase="phase_1", split="discovery", turns=("measured",)):
    """Build :class:`AnalysisObservation` rows for one model, one metric family."""
    selected = _measured_factorial(rows, model_id, phase=phase, split=split, turns=turns)
    return tuple(_observation(row, metric) for row in selected for metric in metrics)


def build_g1_observations(rows, model_id, *, metrics=PRIMARY_METRICS):
    """G1 consumes measured discovery endpoints only, one row per metric."""
    return build_observations(rows, model_id, metrics=metrics, turns=("measured",))


def _neutral_standardization(rows, model_id, *, metrics=PRIMARY_METRICS):
    """The frozen z-scoring baseline: the model's accurate+neutral discovery measured rows."""
    return freeze_neutral_standardization(
        build_observations(rows, model_id, metrics=metrics, turns=("measured", "recovery"))
    )


def build_reversal_rows(rows, model_id, *, metrics=PRIMARY_METRICS):
    """Build G2 rows: matched measured accurate / measured malfunctioning / recovery."""
    frozen = _neutral_standardization(rows, model_id, metrics=metrics)
    measured = {}
    recovery = {}
    eligible = {}
    for row in _measured_factorial(rows, model_id, phase="phase_1", split="discovery", turns=("measured", "recovery")):
...
def build_g5_rows(rows, model_id, *, metrics=PRIMARY_METRICS):
    ...
    """
    out = []
    for row in _measured_factorial(rows, model_id, phase="phase_1", split="discovery"):
        out.append(G5Row(
...
def metric_eligibility(rows, model_id, *, m3_audit_f1=None, phase="phase_1", split="discovery"):
    ...
    an audit F1 is supplied.
    """
    selected = _measured_factorial(rows, model_id, phase=phase, split=split)
    if not selected:
        return tuple(
...
def phase0_screen(rows, *, protocol: Protocol | None = None, screen_task_ids=None, metrics=PRIMARY_METRICS):
    ...
    observations = tuple(
        ...
        for observation in build_observations(
            selected, model_id, metrics=metrics, phase="phase_0", split="discovery", turns=("measured",)
        )
    )
    ...
    selection = analysis_phase0_screen(
        ...
        frozen_model_order=_frozen_model_order(protocol),
        escalated=escalated,
    )
    return Phase0ScreenResult(
        selection, model_ids, screen_task_ids, escalated, rounds,
        _neutral_accuracy(selected, model_ids, set(screen_task_ids)),
        len(selected), len(observations),
    )
...
def render_phase0_markdown(result: Phase0ScreenResult) -> str:
    """Human-readable Phase-0 screen report."""
    ...
    lines += [
        "",
        "## Standardised screen deltas (higher = more instability)",
        "",
        "| model | M1 | M2 | M3 | S | coherent | paired items (M1/M2/M3) |",
        "| --- | ---: | ---: | ---: | ---: | :---: | --- |",
    ]
    for model_id in result.model_ids:
        ...
        lines.append("| `%s` | %s | %s | %s | %s | %s | %s |" % (
            ...
            "yes" if screen.coherent else "no", "/".join(counts),
        ))
    lines += [
        "",
...
def _g1_for_models(observations, metrics):
    """Run G1 once over every supplied model so BH is a single within-phase family."""
    try:
        return g1_adjusted_effects(observations, metrics), None
    except AnalysisInputError as error:
        return {}, "g1_unavailable:" + str(error)


def _g5_for_model(rows, model_id, metrics, *, shuffled=False):
    source = build_g5_rows(rows, model_id, metrics=metrics)
    if not source:
...
def shuffled_nulls(rows, model_ids, *, metrics=PRIMARY_METRICS):
    ...
    for model_id in model_ids:
        try:
            observations.extend(shuffled_feedback_labels(build_g1_observations(rows, model_id, metrics=metrics)))
        except AnalysisInputError as error:
            unavailable[model_id] = "shuffle_unavailable:" + str(error)
    effects, g1_reason = _g1_for_models(tuple(observations), metrics) if observations else ({}, "shuffle_unavailable:no_rows")
    out = {}
    for model_id in model_ids:
        ...
        g5, g5_reason = _g5_for_model(rows, model_id, metrics, shuffled=True)
        if g5_reason:
...
def shuffled_null(rows, model_id, *, metrics=PRIMARY_METRICS):
    ...
    """
    return shuffled_nulls(rows, (model_id,), metrics=metrics)[model_id]
...
def style_effects(rows, style_rows, model_id, metrics, *, smoke_task_ids=G3_SMOKE_TASK_IDS):
    ...
    tasks = tuple(sorted(set(smoke_task_ids)))
    raw = {}
    effects = {}
    for metric in metrics:
        ...
        for style_id in G3_STYLE_IDS:
            ...
            if standardization is None or not standardization.available:
                effects[(metric, style_id)] = StyleEffectEvidence(
                    model_id, metric, style_id, G3_SMOKE_TASK_IDS, G3_NEUTRAL_STYLE_ID,
...
def run_phase1_gates(rows, primary_model, control_model, *, style_rows=None, m3_audit_f1=None):
    ...
    eligibility = {model: metric_eligibility(rows, model, m3_audit_f1=m3_audit_f1) for model in (primary_model, control_model)}
    ...
    observations = tuple(
        ...
        for observation in build_g1_observations(rows, model, metrics=family)
    )
    effects, g1_reason = _g1_for_models(observations, family)
    nulls = shuffled_nulls(rows, (primary_model, control_model), metrics=family)
    analyses = {}
    for model in (primary_model, control_model):
        ...
        null = nulls[model]
        reversal_rows = build_reversal_rows(rows, model, metrics=family)
        real_g2 = {}
        ...
        real_g5, g5_reason = _g5_for_model(rows, model, family)
        if g5_reason:
            reasons.append(g5_reason)
        analyses[model] = ModelAnalysis(
            ...
            len(_measured_factorial(rows, model, phase="phase_1", split="discovery")),
            tuple(reasons),
        )
    core_inputs = {
    ...
    style = style_effects(rows, style_rows, primary_model, family) if style_rows else _freeze({})
    ...
    return Phase1Verdict(primary_model, control_model, family, _freeze(analyses), _freeze(core), summary, style)
